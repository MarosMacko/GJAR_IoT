...
if (WiFi.status() == WL_CONNECTED) { //Check WiFi connection status
    ...
    StaticJsonBuffer<100> JSONid;   //Declaring static JSON buffer
    JsonObject& id = JSONid.createObject(); 
    id["id"] = 0;
    char idMessage [100];
    id.prettyPrintTo(idMessage, sizeof(idMessage));
    ...
    objectD["time"] = "time";
    objectD["humidity"] = "float";
    objectD["temperature"] = "float";
    objectD["voltage"] ="float";
    ...
    StaticJsonBuffer<100> JSONauth;
    JsonObject& auth = JSONauth.createObject();
    auth["user"] = "str";
    auth["password"] = "str";
    char authMessage[100];
    auth.prettyPrintTo(authMessage, sizeof(authMessage));

    StaticJsonBuffer<200> JSONcommand;
    JsonObject& command = JSONcommand.createObject();
    command["token"] = "dd431e8fd96bf3dfabba88184810f9ba521f5f9b6b2d6288572a7599f4143c09";
    command["command"] = "str";
    char commandMessage[200];
    command.prettyPrintTo(commandMessage, sizeof(commandMessage));
    
    Serial.println(idMessage);
    Serial.println(dataMessage);
    Serial.println(errorMessage);
    Serial.println(aliveMessage);
    Serial.println(viewMessage); 
   ...
   1 String payload = http.getString();                                        //Get the response payload
